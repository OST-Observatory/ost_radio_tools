#!/usr/bin/env python3
"""
Plot integrated power spectrum from CSV data file.
This script visualizes the integrated power spectrum of a signal,
showing the cumulative power distribution across different frequencies.
...
- Second column: Integrated power value
...
The script will generate a plot showing the integrated power spectrum in dB scale.
"""
...
def plot_integrated_power_spectrum(
    input_file: str,
    output_file: Optional[str] = None,
    sampling_rate: Optional[int] = None,
    center_frequency: Optional[str] = None
) -> None:
    """Plot integrated power spectrum from processed I/Q data.
    
    Args:
        input_file: Path to the input file containing frequency and integrated power data
        output_file: Path for the output plot file (default: integrated_power_spectrum_plot_<input>.png)
    ...
    The function will:
    1. Load the integrated power spectrum data from the input file
    2. Create a plot with frequency on x-axis and integrated power in dB on y-axis
    3. Add appropriate labels, title, and grid
    4. Save the plot to the output file
    """
    # Load data
    ...
    if center_frequency:
        plt.title(f'Integrated Power Spectrum (Center Frequency: {center_frequency})')
    else:
        plt.title('Integrated Power Spectrum')

    plt.grid(True)

    # Save plot
    if output_file is None:
        output_file = f'integrated_power_spectrum_plot_{os.path.basename(input_file)}.png'
...
def main() -> None:
    ...
    Command line arguments:
        ...
        --center-frequency, -f: Center frequency in Hz (default: extracted from filename)
    """
    parser = argparse.ArgumentParser(description='Plot integrated power spectrum from processed I/Q data')
    parser.add_argument('input_file', help='Input file containing frequency and integrated power data')
    parser.add_argument('--output', '-o', help='Output file for the plot (default: integrated_power_spectrum_plot_<input>.png)')
    ...
    plot_integrated_power_spectrum(args.input_file, args.output, args.sampling_rate, args.center_frequency)
